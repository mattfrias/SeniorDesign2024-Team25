...
def apply(args):
  ...
  try:
    ...
    print(f"Failed to apply resource of type '{resource_type}' from file '{filename}'.")
...
def show(args):
  ...
  try:
    if resource == "nodes":
      ...
      subprocess.run(['kubectl', 'get', 'pods', '-n', 'default', '-o', 'custom-columns=NAME:.metadata.name,PRIORITY:.spec.priorityClassName'])
    elif resource == "pods":
      subprocess.run(['kubectl', 'get', 'pods', '--output=custom-columns=NAMESPACE:.metadata.namespace,NAME:.metadata.name,CPU-REQUEST:.spec.containers[].resources.requests.cpu,CPU-LIMIT:.spec.containers[].resources.limits.cpu,MEMORY-REQUEST:.spec.containers[].resources.requests.memory,MEMORY-LIMIT:.spec.containers[].resources.limits.memory,AGE:.metadata.creationTimestamp'])
  except subprocess.CalledProcessError:
    if resource == "nodes":
